...
const buildPaymentDetails = (order, paymentMethod) => {
...
exports.createOrder = async (req, res) => {
  try {
    ...
    Object.assign(newOrder, buildPaymentDetails(newOrder, normalizedPayment));
    await newOrder.save();
...
exports.getMyOrders = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      count: orders.length,
      orders: orders.map((order) => mapOrderForFrontend(order)),
    });
  } catch (error) {
