...
export default function DemoLoginPanel({ onDemoLogin }: DemoLoginPanelProps) {
  ...
  return (
    <div className={`fixed left-4 md:left-8 top-1/2 -translate-y-1/2 z-50 transition-all duration-500 ease-out flex items-center ${
      isOpen ? 'translate-x-0' : '-translate-x-[calc(100%-24px)] md:-translate-x-[calc(100%-28px)]'
    }`}>
      {/* Main Glass Panel */}
      ...
      >
        {isOpen ? <ChevronLeft size={16} /> : <Key size={14} className="animate-pulse" />}
      </button>
    </div>
  );
}
